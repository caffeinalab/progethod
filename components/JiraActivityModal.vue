    ...
    <div v-if="value" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="close">
      ...
      <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto px-5 pb-5 min-h-0">
          ...
          <div v-else-if="issues.length === 0" class="py-10 text-center text-sm text-gray-400">
            {{ $t('jira.no_activity') }}
          </div>
            ...
            <div v-for="group in filteredGroups" :key="group.project" class="mb-4 last:mb-0">
              ...
              <ul class="space-y-1">
                <li
                  ...
                  :class="{ 'bg-green-50': addedKeys.includes(issue.key) }"
                  @click="selectIssue(issue)"
                >
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                      <code class="text-xs font-semibold text-indigo-600">{{ issue.key }}</code>
                      <span class="relative">
                        <button
